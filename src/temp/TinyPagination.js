...
class TinyPagination extends React.Component {
  ...
  renderPreBtn(totalBtns, selectedPageId){
    let {preKey} = this.props;
    if(selectedPageId > 1){
      return this.props.renderBtnNumber(preKey);
    }
  }

  renderNextBtn(totalBtns, selectedPageId){
    let {nextKey} = this.props;
    if(selectedPageId !== totalBtns){
      return this.props.renderBtnNumber(nextKey);
    }
  }
...
TinyPagination.propTypes = {
  ...
  preKey: PropTypes.string.isRequired,
